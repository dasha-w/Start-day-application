...
TEMP_THRESHOLD = {"cold" : 5, "chilly" : 10, "mild" : 15, "mild_warm" : 20, "warm" : 25}
...
def get_forecast_descriptives(filtered_hourly: list[dict[str, str|float|int]]) -> dict:
    ...
    """
    if not filtered_hourly: # If list is empty - prevents ZeroDivisionError
        print(f"{Fore.RED}Warning:{Fore.RESET} No hourly data provided.")
        return {
            "min_temp": 0.0, "max_temp": 0.0, "avg_temp": 0.0, "temp_range": 0.0,
            "feel_min_temp": 0.0, "feel_max_temp": 0.0, "feel_avg_temp": 0.0,
            "avg_wind": 0.0, "max_uv": 0.0, "tot_precip": 0.0,
            "avg_chance_rain": 0.0, "will_it_rain": False, "will_it_snow": False,
    ...
    descriptives = {
        ...
        "feel_min_temp": min(hour["feelslike_c"] for hour in filtered_hourly),
        "feel_max_temp": max(hour["feelslike_c"] for hour in filtered_hourly),
        "feel_avg_temp": round(sum(hour["feelslike_c"] for hour in filtered_hourly) / len(filtered_hourly), 2),
        # --- Wind & UV ---
        "avg_wind": round(sum(hour["wind_kph"] for hour in filtered_hourly) / len(filtered_hourly), 2),
        "max_uv": max(hour["uv"] for hour in filtered_hourly),
        # --- Rain ---
        "tot_precip": sum(hour["precip_mm"] for hour in filtered_hourly),
        "avg_chance_rain": round(sum(hour["chance_of_rain"] for hour in filtered_hourly) / len(filtered_hourly), 2),
        "will_it_rain": True if sum(hour["will_it_rain"] for hour in filtered_hourly) > 0 else False,
        # --- Snow ---
        "will_it_snow": True if sum(hour["will_it_snow"] for hour in filtered_hourly) > 0 else False
    }
    ...
    # return the top 1 most frequent occurrence in tuple -> [0][0] access first item in first tuple
...
def generate_clothing_advice(descriptives: dict) -> dict:
    ...
    elif avg_temp <= TEMP_THRESHOLD["mild_warm"]:
