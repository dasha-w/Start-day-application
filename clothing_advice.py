...
def get_forecast_descriptives(
    filtered_hourly: list[dict[str, str | float | int]],
) -> dict:
    """
    ...
    # Je zou dit ook direct in 1 variable statement kunnen doen. Dan hoef je niet steeds descriptives['key'] te schrijven.
    # descriptives = {
    #     'min_temp': min(hour['temp_c'] for hour in filtered_hourly),
    #     'max_temp': max(hour['temp_c'] for hour in filtered_hourly),
    #     'avg_temp': round(sum(hour['temp_c'] for hour in filtered_hourly) / len(filtered_hourly), 2),
    #     'temp_range': round(max(hour['temp_c'] for hour in filtered_hourly) - min(hour['temp_c'] for hour in filtered_hourly), 2)
    # }
    ...
    descriptives["avg_temp"] = round(
        sum(hour["temp_c"] for hour in filtered_hourly) / len(filtered_hourly), 2
    )
    descriptives["temp_range"] = round(
        descriptives["max_temp"] - descriptives["min_temp"], 2
    )
    ...
    descriptives["feel_avg_temp"] = round(
        sum(hour["feelslike_c"] for hour in filtered_hourly) / len(filtered_hourly), 2
    )

    # --- Wind & UV ---
    descriptives["avg_wind"] = round(
        sum(hour["wind_kph"] for hour in filtered_hourly) / len(filtered_hourly), 2
    )
    ...
    descriptives["most_common_condition"] = count_conditions.most_common(1)[0][
        0
    ]  # if multiple have highest count first in alphabet is returned

    # --- Rain ---
    descriptives["tot_precip"] = round(
        sum(hour["precip_mm"] for hour in filtered_hourly)
    )  # krijg to echt float met veel achter komma zonder round()
    descriptives["avg_chance_rain"] = round(
        sum(hour["chance_of_rain"] for hour in filtered_hourly) / len(filtered_hourly)
    )
    descriptives["will_it_rain"] = (
        True if sum(hour["will_it_rain"] for hour in filtered_hourly) > 0 else False
    )

    # --- Snow ---
    descriptives["will_it_snow"] = (
        True if sum(hour["will_it_snow"] for hour in filtered_hourly) > 0 else False
    )
...
def generate_clothing_advice(descriptives: dict) -> dict:
    ...
    if avg_temp <= TEMP_THRESHOLD["cold"]:
        advice["temp"] = (
            f"It's going to feel {Fore.BLUE}cold{Style.RESET_ALL} today. Bundle up!\n"
            ">    Wear warm layers, a thick coat, hat and scarf."
        )
    elif avg_temp <= TEMP_THRESHOLD["chilly"]:
        advice["temp"] = (
            f"It's going to feel {Fore.BLUE}chilly{Style.RESET_ALL} today. Dress warmly.\n"
            ">    Wear a warm coat or a sweater and light jacket. Consider bringing a scarf."
        )
    elif avg_temp <= TEMP_THRESHOLD["mild"]:
        advice["temp"] = (
            f"It's going to feel {Fore.BLUE}mild{Style.RESET_ALL} today.\n"
            ">    Wear layers and consider a transitional jacket."
        )
    elif avg_temp <= TEMP_THRESHOLD["mild/warm"]:  # 15-20
        advice["temp"] = (
            f"It's going to feel {Fore.BLUE}mild/ warm{Style.RESET_ALL} today. Layering is advisable.\n"
        ...
        advice["temp"] = (
            f"It's going to feel {Fore.BLUE}warm{Style.RESET_ALL} today.\n"
            f">   T-shirt weather! Wear light clothing."
        )
    else:  # >25C
        advice["temp"] = (
            f"It's going to feel {Fore.BLUE}hot{Style.RESET_ALL} today!\n"
    ...
    if temp_range > TEMP_RANGE_THRESHOLD["high"]:
        advice["range"] = (
            f"There will be a {Fore.BLUE}big temperature{Fore.RESET} range today (range: {temp_range}{DEGREE_SYMBOL}C). \n"
            f">   Layering is key!\n"
        )
    elif temp_range > TEMP_RANGE_THRESHOLD["medium"]:
        advice["range"] = (
            f"Expect {Fore.BLUE}some temperature{Fore.RESET} variation today (range: {temp_range}{DEGREE_SYMBOL}C).\n"
        )
    else:
        advice["range"] = (
            f"Temperature will be {Fore.BLUE}fairly consistent{Fore.RESET} throughout the day.\n"
        )
    ...
    if not descriptives["will_it_rain"]:
        if rain_chance > RAIN_CHANCE_THRESHOLD:  # if 10, chance of rain higher than 10%
            advice["rain"] = (
                f"It should {Fore.BLUE}not rain{Fore.RESET} today. Although there is a {rain_chance}% "
                f"chance of rain.\n"
        ...
        if rain_tot < RAIN_MM_THRESHOLD:
            advice["rain"] = (
                f"There will be {Fore.BLUE}some rain{Fore.RESET} today.\n"
            ...
            advice["rain"] = (
                f"It {Fore.BLUE}will rain{Fore.RESET} today. The chance of rain is {rain_chance}% "
    ...
    if descriptives["will_it_snow"]:
        advice["snow"] = (
            f"It's going to {Fore.BLUE}snow{Fore.RESET} today! \n"
    ...
    if avg_wind > WIND_THRESHOLD["windy"]:
        advice["wind"] = (
            f"It's going to be {Fore.BLUE}windy{Fore.RESET} today with {avg_wind}kph winds. \n"
            f">   A windbreaker might come in handy."
        )
    elif avg_wind > WIND_THRESHOLD["strong_wind"]:
        advice["wind"] = (
            f"There wil be {Fore.BLUE}strong winds{Fore.RESET} of {avg_wind} kph. \n"
    ...
    if uv_index > UV_THRESHOLD["high"]:
        advice["uv"] = (
            start_advice
            + f">   Protect your skin with sunscreen and a hat! The sun is fierce today!"
        )
...
def print_clothing_advice(
    location: str, date: str, descriptives: dict, advice: dict
) -> None:
    """
    ...
    # advice temp
    print(
        f"The average {Fore.BLUE}temperature{Fore.RESET} will {Style.BRIGHT}feel like{Style.RESET_ALL} "
...
def display_clothing_advice(weather) -> None:
    ...
    # Deze zou je ook kunnen verplaatsen binnen de print functie. Dan is de overall flow iets netter
